...
onMounted(() => {
  ...
  if (token) {
    ...
    if (urlParams.get('error') === 'unauthorized') {
      errorMessage.value = "Accesso negato: non fai parte di nessuna Gilda autorizzata."
    }
  }
})
</script>
...
<template>
  <div class="flex flex-col items-center justify-center min-h-[50vh]">
    ...
    <div v-else class="glass-panel p-6 md:p-10 text-center max-w-md w-full">
      <div v-if="errorMessage" class="bg-red-900/30 border border-red-800 text-red-200 p-3 rounded mb-4 text-sm font-mono">
        {{ errorMessage }}
      </div>
      <h2 class="font-orbitron text-2xl mb-6 text-gray-300">{{ t('login.adminAccessRequired') }}</h2>
